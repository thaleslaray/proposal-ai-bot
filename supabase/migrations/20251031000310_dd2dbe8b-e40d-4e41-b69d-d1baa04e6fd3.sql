-- Atualizar função get_users_with_details para aceitar filtros de source e role
...
BEGIN
  ...
  WHERE 
    ...
    AND (
      ...
      COALESCE(ua.utm_source, (SELECT inferred_source FROM infer_utm_from_acquisition(ua.referrer, ua.landing_page))) = source_filter
    )
    AND (
      role_filter IS NULL OR
      role_filter = 'all' OR
      COALESCE(ur.role::text, 'free') = role_filter
    )
  GROUP BY 
    au.id, au.phone, au.email, au.raw_user_meta_data, au.updated_at, au.created_at,
